...
public class MinAndMax_2357 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        long[] maxtree = new long[tree_size];
        ...
        for (int i = 0; i < m; i++) {
            ...
            bw.write(minquery(mintree, 1, 0, n - 1, left - 1, right - 1) + " "
                    + maxquery(maxtree, 1, 0, n - 1, left - 1, right - 1) + "\n");
        }
    ...
    static void maxinit(long[] a, long[] tree, int node, int start, int end) {
        if (start == end) {
            tree[node] = a[start];
        } else {
    ...
    static long maxquery(long[] tree, int node, int start, int end, int left, int right) {
        if (left > end || right < start) {
            return -1;
        }
        if (left <= start && end <= right) {
        ...
        long lmax = maxquery(tree, node * 2, start, (start + end) / 2, left, right);
        long rmax = maxquery(tree, node * 2 + 1, (start + end) / 2 + 1, end, left, right);
        if (lmax == -1) {
            return rmax;
        } else if (rmax == -1) {
            return lmax;
